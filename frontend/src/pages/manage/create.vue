    ...
    <v-row class="justify-center">
      <v-col cols="12" sm="8" md="6" lg="4">
        <v-card variant="flat" class="pa-4">
          <v-card-title class="text-center text-h5 text-primary">Add product</v-card-title>
          <v-card-item>
            <v-sheet>
              <v-form @submit.prevent="submit">
                ...
                <v-label class="pl-4 text-subtitle-1 text-medium-emphasis text-primary opacity-90 font-weight-medium"
                         text="Description" />
                <v-textarea maxlength="250" counter density="compact" placeholder="Enter product description"
                            variant="outlined" rounded v-model="form.description" :rules="rules.description"
                            class="mb-1"/>

                <v-label class="pl-4 text-subtitle-1 text-medium-emphasis text-primary opacity-90 font-weight-medium"
                         text="Price" />
                <v-text-field density="compact" placeholder="Enter product price" variant="outlined" prefix="$"
                              rounded type="number" step="0.01" v-model="form.price" :rules="rules.price" class="mb-1"/>

                <v-label class="pl-4 text-subtitle-1 text-medium-emphasis text-primary opacity-90 font-weight-medium"
                         text="Quantity" />
                <v-text-field density="compact" placeholder="Enter product quantity" variant="outlined" rounded
                ...
                <v-btn type="submit" block variant="tonal" color="primary" size="large" class="my-2" rounded>
                  Add
                </v-btn>
              </v-form>
            </v-sheet>
